...
function llenar_table(d) {
  ...
  d.map(e => {
      ...
      <td>${e['CD']}</td>
      <td>${e['FOTOGRAFÍA']}</td>
      <td>${formatoFecha(numeroAFecha(+e['FECHA REVISIÓN']))}</td>
...
function asignar_niveles_accion(datos) {
  datos.map((solicitud) => {
    ...
    documentos.map((doc) =>{
      let noIncidencia = solicitud[doc];
      let accion = inconsistencia_documento_identidad[noIncidencia].accion;
      if (!_nivel_acciones.includes(accion)) {
        _nivel_acciones.push(accion);
      }
    });
    ...
    let accion_foto = inconsistencia_documento_identidad[solicitud['FOTOGRAFÍA']].accion;
    if (!_nivel_acciones.includes(accion_foto)) {
      _nivel_acciones.push(accion_foto);
    }
    ...
    _nivel_acciones.map((accion) =>{
      solicitud["OBSERVACIONES(SEÑALAR EL SOPORTE DOCUMENTAL)"] += inconsistencia_soporte[accion];
    });

    let fechaRevision = numeroAFecha(solicitud['FECHA REVISIÓN']);
    let dia = fechaRevision.getDay();
    if( [5,6].includes(dia)){
  ...
  function formatoFecha(fecha) {
    fecha = `${dosD(fecha.getDate())}/${dosD(fecha.getMonth()+1)}/${fecha.getFullYear()}`;
    return fecha
  }


  function dosD(numero) {
